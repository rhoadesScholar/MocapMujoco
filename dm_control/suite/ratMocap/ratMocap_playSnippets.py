...
# flags.DEFINE_float('fpsIn', 300, 'Frame step of recording')
flags.DEFINE_float('fpsOut', 30, 'Frame step for plotting/playback')
flags.DEFINE_integer('dpi', 300, 'DPI for mp4')
flags.DEFINE_string('model_filename', 'models\\ratMocap.xml', 'filename for model.')
flags.DEFINE_string('outName', 'dataOutput\\ratMocapOutExample', 'filename for saving.')
flags.DEFINE_string('name', '', 'filename for model.')
flags.DEFINE_bool('record', True, 'Whether to write a video file.')     
flags.DEFINE_bool('mocap', True, 'Whether render mocap markers.')        
flags.DEFINE_bool('play', False, 'Whether to play clip from python.')                        
...
def main(unused_argv):
    ...
    max_frame = min(FLAGS.max_frame, data.qVid.shape[0] - 1)
    ...
    if FLAGS.record:
        ...
        metadata = dict(title= FLAGS.model_filename + ': ' + FLAGS.name, artist='Jeff Rhoades/Jesse Marshall/DeepMind')
        ...
        with writer.saving(fig, FLAGS.outName + '.mp4', dpi=FLAGS.dpi):
            for i in range(video.shape[0]):
                img.set_data(video[i])
                plt.draw()
                writer.grab_frame()
